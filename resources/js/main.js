...
document.getElementById("add").addEventListener("click", function () {
  ...
  if (value.includes("@paste")) {
    ...
    if (Array.isArray(parsedData.todo) && Array.isArray(parsedData.completed)) {
      ...
      data.todo = parsedData.todo.map((item) => ({
        value: item,
        category: "Principale",
      }));
      data.completed = parsedData.completed.map((item) => ({
        value: item,
        category: "Principale",
      }));
    } else {
  ...
  .addEventListener("click", function (e) {
    ...
    if (category) {
      ...
      data.todo.forEach(function (todoItem) {
        if (todoItem.category === category) {
          data.todo.splice(data.todo.indexOf(todoItem), 1);
        }
      });

      data.completed.forEach(function (completedItem) {
        if (completedItem.category === category) {
          data.completed.splice(data.completed.indexOf(completedItem), 1);
        }
      });
      var items = document.querySelectorAll("#todo li, #completed li");
      [].forEach.call(items, function (item) {
      ...
      document.getElementById("item").placeholder =
        "Entrer une tâche pour Principale";
    }
    dataObjectUpdated();
  });
...
function dataObjectUpdated() {
  ...
  categoryList.addEventListener("click", function (event) {
    if (event.target.tagName === "LI") {
      ...
      document.getElementById("item").placeholder =
        "Entrer une tâche pour " + category;
...
function editItem(item) {
  ...
  textElement.innerHTML =
    '<input type="text" class="edit-item" value="' + text + '">';
